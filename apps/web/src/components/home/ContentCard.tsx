...
const ContentCard = ({ content, onInfoClick, onPlayClick }: ContentCardProps) => {
  ...
  return (
    <div
      className="group relative min-w-[180px] aspect-[3/4] cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-accent-500/30"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Thumbnail with soft corners and warm glow */}
      <OptimizedImage
        src={content.thumbnail_url}
        alt={content.title}
        className="w-full h-full object-cover rounded-xl shadow-lg group-hover:shadow-xl group-hover:shadow-accent-500/40 transition-shadow duration-300"
        format="auto"
        placeholder="blur"
      />
      ...
      {isHovered && (
        <div className="absolute inset-0 bg-gradient-to-t from-warm-charcoal-100 via-warm-charcoal-100/95 to-warm-charcoal-100/80 rounded-xl p-3 flex flex-col justify-between backdrop-blur-sm ring-2 ring-accent-500/20">
          {/* Title & Metadata */}
          <div>
